...
const IntelligenceSchema = z.object({
  ...
  keywordResearch: z.array(z.string()).min(1).max(15).describe("5-10 high-intent search keywords for this affiliate offer"),
  marketingAngle: z.string().describe("One powerful marketing angle sentence for this offer"),
  personas: z.array(PersonaSchema).min(1).max(8).describe("3-4 distinct buyer personas for this offer"),
  contentSuggestions: z.array(z.string()).min(1).max(12).describe("5-6 specific content ideas (video titles, post hooks, email subjects)"),
  targetPlatforms: z.array(z.string()).min(1).max(8).describe("3-5 best traffic platforms in order of priority"),
  ...
  complianceRules: z.array(z.string()).min(1).max(10).describe("3-5 specific compliance rules for this affiliate program and platform"),
